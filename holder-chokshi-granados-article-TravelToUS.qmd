...
selected_spikes <-
  ...
  ungroup() |>
  slice(.by = region, 1:no[1]) |> # Use no[1] to ensure it's scalar if regions have different 'no'
  select(-no) |>
  bind_rows(
...
gradient_data <- expand.grid(x = x_seq, y = y_seq) |>
  ...
  mutate(
    ...
    alpha_value = if_else(within_countries, 1.0, 0.0) # this is for gradient_data alpha, not spikes
  )
...
final_plot <-
  ...
  ggtitle('**Big drops along the border**')
