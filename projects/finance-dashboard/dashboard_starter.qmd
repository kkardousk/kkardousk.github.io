...
def plot_single_comparison(data, title="Stock Price Comparison"):
    ...
    fig.update_layout(
        title={
            ...
            'x': 0.5,
            'xanchor': 'center',
            'font': {'size': 16, 'family': 'Merriweather, Georgia, serif', 'color': '#333333'}
        }
    )
    return fig
...
```{python}
def create_stock_calendar_grid(df, stocks, months, vmin=-5, vmax=5, colorscale='viridis'):
    ...
    for idx, m in enumerate(unique_months):
        ...
        if len(dmonth) > 0:
            ...
            heatmap = go.Heatmap(
                ...
                colorbar=dict(
                    title="% Change in Daily Stock Price",
                    orientation="h",
                    titleside="top",
                    len=0.4,
                    thickness=15,
...
# crate a function tht loops through all stocks to display calendar view
def calendar_stock_display(stocks):
...
for ticker in tickers:
  calendar_stock_display([ticker])
  
```

From above calendar plots, the idea is to tease out, if any, day of the week/week/or month seasonality at once. 
A few patterns stand out:
  - Tesla's stock tends to signficantly pick up toward the end of the week, i.e. Fridays, and regardless of the month.
