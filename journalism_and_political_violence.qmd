...
**Sometimes, "a [plot] is worth a thousands words".**
:::
```{r}
library(tidyverse)
library(ggiraph)
...
create_waffle_positions <- function(area_name, kills, complete_sequence_7_rows, padding_needed) {
  ...
  positions <- tibble(
    ...
    filter(type == "deaths")  # only keep death tiles for visualization
...
# create individual waffle charts with ggiraph
create_interactive_waffle <- function(area_name, kills, no_of_rows) {
  
  # filter positions for this area
  area_positions <- all_positions |> filter(area == area_name)
  # determine svg path based on area
  svg_path <- if(area_name == 'Gaza Strip') '/Users/kardouskarrim/Desktop/person-svgrepo.svg' else '/Users/kardouskarrim/Desktop/person-svgrepo-orange.svg'
  ...
  p <- ggplot(area_positions, aes(x = col_pos, y = row_pos)) +
    # use geom_point_interactive for interactivity
    geom_point_interactive(
      aes(data_id = area),
      size = 15,  # adjust size as needed
      alpha = 0,  # make points transparent since we'll overlay svgs
      color = "transparent"
    ) +
    scale_x_continuous(limits = c(0.5, 7.5), expand = c(0, 0)) +
    scale_y_continuous(limits = c(0.5, no_of_rows + 0.5), expand = c(0, 0)) +
    coord_fixed() +
    theme_void() +
    theme(
      plot.title = element_text(
        hjust = if_else(area_name == 'India', 0.4, 0.5),
        family = 'franklin',
        vjust = 0.1,
        size = 17,
        face = 'bold'
    ...
    # add area name label below the waffle chart
    annotate(
      'text',
      x = 4,  
      y = -0.5,
    ...
    # extend coordinate system to show labels below
    coord_cartesian(clip = "off", ylim = c(-1, no_of_rows + 0.5))
  
  # add svg annotations for each death tile
  svg_annotations <- map(1:nrow(area_positions), ~ {
...
# align charts (same logic as before)
waffle_charts_aligned <- map2(waffle_charts, waffle_data$no_of_rows, ~{
  max_rows <- max(waffle_data$no_of_rows)
  spacer_height <- (max_rows - .y) * 7
  
  if(spacer_height > 0) {
    spacer <- plot_spacer() + theme(plot.margin = margin(0, 0, 3, 0))
    spacer / .x + plot_layout(heights = c(spacer_height, .y * 7))
  } else {
    .x
  }
})
...
# create main plot
main_plot <- wrap_plots(waffle_charts_aligned, nrow = 1)

## using cowplot instead of patchwork for better label control
...
# create all waffle charts (no alignment/spacers this time)
waffle_charts <- pmap(
  list(waffle_data$area, waffle_data$kills, waffle_data$no_of_rows),
  create_interactive_waffle
)

# create main plot without spacer alignment
main_plot <- wrap_plots(waffle_charts, nrow = 1) +
  plot_annotation(
    ...
    subtitle = "7 October 2023 — 29 August 2025<br>Journalists killed <img src='/person-grey.png' width='16' height='16'> = 1",
    caption = 'Data and Plot Replicated from ACLED',
    theme = theme(
      plot.title = element_text(size = 20, hjust = 0, family = 'franklin', face = "bold", color = '#2D2D2D', margin = margin(t = -5, b = 5)),
      plot.subtitle = ggtext::element_markdown(size = 16, hjust = 0, family = 'franklin', color = '#5A5A5A', margin = margin(t = -4, b = 4)),
      plot.caption.position = 'panel',
      plot.caption = element_text(size = 12, hjust = 0, family = 'franklin', color = '#5A5A5A')
    )
  )
