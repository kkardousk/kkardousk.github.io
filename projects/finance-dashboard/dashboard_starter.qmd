...
title: |
  <div class="custom-title-block" style="font-size: 1.2em;">
    <span style="color:#000000;">Stock Market Analysis Report (Python)</span><br>
...
# clear existing handlers and add only one
if logger.handlers:
...
::: {.text-justify}
As we can see from above, Netflix stock 'overshadows' any subtle variations in the stock prices for the remaining companies/stocks, simply because of the difference in scales/range of values. <br>
One way to remedy that is by clicking on NFLX tile in the legend, this will deselect the Netflix Series - pun intended - from the graph; a native functionality in `{Plotly}`
Another way is to be faceting on companies, essentially a '`GROUP BY TICKER`' but for plots. This is what is shown next.
:::
...
::: {.text-justify}
Overall, and despite the dips and volatility, and while a few grew more than others, all 6 stocks have risen in value over the trailing 6 month period.
Speaking of volatility, let's gather more stats around stock price evolution and tease out more insights overall. 
:::
